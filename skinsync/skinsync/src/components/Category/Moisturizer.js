...
const Moisturizer = () => {
  ...
  const fetchProducts = async () => {
    try {
      // Fetch only products with category 'moisturizer'
      const response = await axiosInstance.get("http://localhost:5000/api/products/all?category=moisturizer");
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      <div className="bg-white rounded-lg shadow-lg p-8">
        <h2 className="text-2xl font-bold mb-4">Moisturizers</h2>
        ...
        <div>
          {products.length > 0 ? (
            ...
            <p className="text-gray-500 text-center py-4">No moisturizers available</p>
          )}
        </div>
      </div>
      <style>{`
