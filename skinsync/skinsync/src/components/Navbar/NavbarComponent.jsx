    ...
    <header className="flex items-center justify-between p-4 border-b border-gray-200 bg-white shadow-md">
      <div className="flex items-center space-x-4">
        <Link to="/" className="flex items-center py-2">
          <span className="font-semibold text-2xl" style={{ color: '#8A5647' }}>SkinSync</span>
        </Link>
      </div>
      <nav className="hidden md:flex items-center space-x-6">
        ...
        {isLoggedIn ? (
          <button 
            ...
            style={{ color: '#8A5647' , borderColor:'#8A5647'}}
          >
            Logout
          </button>
        ) : (
          <>
            <Link to="/login" className="px-4 py-2 border rounded-md" style={{ color: '#8A5647' , borderColor:'#8A5647'}}>Login</Link>
          ...
          <ul>
            ...
            {isLoggedIn ? (
              <li>
                <button 
                  onClick={handleLogout} 
                  className="px-4 py-2 border rounded-md" style={{ color: '#8A5647' , borderColor:'#8A5647'}}                >
                  Logout
                </button>
              </li>
            ) : (
