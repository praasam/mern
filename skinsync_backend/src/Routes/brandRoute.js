const express = require("express");
const router = express.Router();
const auth = require("../Middleware/authMiddleware");
const authorizeRole  = require("../Middleware/authorizationMiddleware");
const {
  addBrand,
  updateBrand,
  getBrand,
  deleteBrand,
  getBrands,
} = require("../Controllers/brandControllers");

/**
 * @description To create categories
 * @api /api/category/create
 * @access Private
 * @type POST
 * @return response
 */

router.post("/create", auth, authorizeRole("admin"), addBrand);

/**
 * @description To get all categories
 * @api /api/category/all
 * @access Public
 * @type GET
 * @return response
 */
router.get("/all", getBrands);

/**
 * @description To update categories by id
 * @api /api/category/update/:id
 * @access Private
 * @type PUT
 * @return response
 */
router.patch("/update/:id", auth, authorizeRole("admin"), updateBrand);

/**
 * @description To delete categories by id
 * @api /api/category/delete/:id
 * @access Private
 * @type DELETE
 * @return response
 */
router.delete("/delete/:id", auth, authorizeRole("admin"), deleteBrand);

module.exports = router;